...
class NetworkMonitor:
    ...
    def ping_target(self, host):
        ...
        if not times:
            return {"host": host, "latency": None, "packet_loss": 100.0}
        return {"host": host, "latency": mean(times), "packet_loss": packet_loss}
    ...
    def check(self):
        ...
        valid_latencies = [result["latency"] for result in ping_results if result["latency"] is not None]
        latency = round(mean(valid_latencies), 1) if valid_latencies else 999.0
        ...
        return NetworkHealth(status, score, latency, packet_loss, dns_ok, details)
